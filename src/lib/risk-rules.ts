...
export function extractAmounts(text: string): ExtractedAmount[] {
  ...
  // 2026-09-23 리뷰 C1: 종전 `[^0-9]*([0-9,]+…)` 는 쉼표에서 간격·금액이 겹쳐 제곱("계약금액" + 쉼표
  // 10만 자 → 13.5초, 업스트림 호출 없이 이벤트 루프 정지)이었다. 간격 500자·숫자 시작으로 묶는다(200자는 괄호 삽입구가 긴 한 문단짜리 계약서에서 짝을 놓쳤다, 독립 리뷰).
  // 정상 문서 결과는 같고(테스트 기준값) 쉼표로 시작하는 쓰레기 금액(", 원")만 사라진다.
  const patterns: { label: string; re: RegExp }[] = [
    { label: "계약금액", re: /계약\s*금액[^0-9]{0,500}([0-9][0-9,]*\s*(?:원|만원|억원|백만원))/g },
    { label: "위약금", re: /위약금[^0-9]{0,500}([0-9][0-9,]*\s*(?:원|만원|억원|백만원|%))/g },
    { label: "보증금", re: /보증금[^0-9]{0,500}([0-9][0-9,]*\s*(?:원|만원|억원|백만원))/g },
    { label: "월세/임대료", re: /(?:월세|임대료|월\s*임대료)[^0-9]{0,500}([0-9][0-9,]*\s*(?:원|만원|억원|백만원))/g },
    { label: "손해배상", re: /손해배상[^0-9]{0,500}([0-9][0-9,]*\s*(?:원|만원|억원|백만원|%))/g },
    { label: "투자금", re: /투자금[^0-9]{0,500}([0-9][0-9,]*\s*(?:원|만원|억원|백만원))/g },
    { label: "로열티", re: /(?:로열티|사용료)[^0-9]{0,500}([0-9][0-9,]*\s*(?:원|만원|억원|백만원|%))/g },
    { label: "공사대금", re: /(?:공사대금|도급금액)[^0-9]{0,500}([0-9][0-9,]*\s*(?:원|만원|억원|백만원))/g },
    { label: "기성금", re: /기성금[^0-9]{0,500}([0-9][0-9,]*\s*(?:원|만원|억원|백만원|%))/g },
    // 일반 금액 (금 OOO원)
...
export function extractPeriods(text: string): ExtractedPeriod[] {
  ...
  // 2026-09-23 리뷰 C1: extractAmounts 와 같은 이유로 간격을 500자로 묶는다("해지통보" 반복 10만 자 0.9초)
  const patterns: { label: string; re: RegExp }[] = [
    { label: "계약기간", re: /계약\s*기간[^0-9]{0,500}([0-9]+\s*(?:년|개월|일|주))/g },
    { label: "해지 통보 기간", re: /(?:해지|해약)\s*(?:통보|통지|고지)[^0-9]{0,500}([0-9]+\s*(?:일|개월|주)\s*(?:전|이전)?)/g },
    { label: "비밀유지 기간", re: /비밀\s*유지\s*기간[^0-9]{0,500}([0-9]+\s*(?:년|개월))/g },
    { label: "경업금지 기간", re: /(?:경업금지|전직금지)\s*기간[^0-9]{0,500}([0-9]+\s*(?:년|개월))/g },
    { label: "보증기간", re: /(?:보증|하자보수)\s*기간[^0-9]{0,500}([0-9]+\s*(?:년|개월|일))/g },
    { label: "공사기간", re: /공사\s*기간[^0-9]{0,500}([0-9]+\s*(?:년|개월|일))/g },
...
// 2026-09-23 리뷰 C1: `.*\d+` 는 숫자 덩어리에서 제곱("해지통보" + 숫자 10만 자 → 24.4초)이었다.
// 간격을 500자로 묶는다. 존재 판정이라 `\d+` 앞자리는 간격이 흡수해 `\d{1,4}` 여도 같다.
// 줄을 넘던 `\s*` 는 `(?:\s*(?!.)\s)?`(줄바꿈으로 끝나는 공백)로 옮겨 간격과 겹치지 않게 한다.
const CONFLICT_RULES: ConflictRule[] = [
  {
    type: "해지통보 vs 즉시해지",
    description: "사전통보 기간이 있는데 즉시해지도 가능 -- 어느 조항이 우선인지 불명확",
    patternA: /(?:해지|해약)\s*(?:통보|통지|고지)(?:\s*(?!.)\s)?.{0,500}?\d{1,4}\s*(?:일|개월)\s*(?:전|이전)/,
    patternB: /즉시\s*해지|사전\s*통지\s*없이\s*해지|통보\s*없이\s*해지/,
  },
  {
    type: "자동갱신 vs 계약기간 확정",
    description: "계약기간이 확정인데 자동갱신 조항도 있음 -- 종료 시점 혼란",
    patternA: /계약\s*기간.{0,500}?\d{4}[.\-/]\d{1,2}[.\-/]\d{1,2}\s*(?:까지|만료)/,
    patternB: /자동\s*(?:갱신|연장)/,
  },
  {
    type: "면책 vs 손해배상",
    description: "면책 조항과 손해배상 조항이 공존 -- 면책 범위와 배상 의무가 충돌할 수 있음",
    patternA: /일체의?\s*책임.{0,500}?(?:지지|않|없)/,
    patternB: /손해배상.{0,500}?(?:하여야|책임|배상)/,
  },
  {
    type: "독점 vs 제3자 허용",
    description: "독점 조항인데 제3자 사용 허용도 언급 -- 독점 범위 불명확",
    patternA: /독점적\s*(?:사용|실시|라이선스)/,
    patternB: /제3자.{0,500}?(?:허용|허락|사용할 수 있|재실시)/,
  },
]

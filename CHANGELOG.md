...
- **조문 본문의 개정 표기가 지워지던 문제**: `cleanHtml` 이 `<[^>]+>` 로 태그를 지우면서 `<개정 2012.8.1.>`·`<신설 2005.12.29>`·
  `부칙 <1999.01.11>` 같은 꺾쇠 표기까지 없앴다(민법 전문에서만 149개). 행위시법 판단의 근거라 영문 태그·주석만 지우고
  한글 꺾쇠 표기는 남긴다. `applicable_law` 부칙 머리글은 원문 줄 대신 공포번호·일자로 표준 표기(YYYY.MM.DD)를 조립한다

### Security

- **setup 마법사가 커밋되는 설정 파일에 API 키를 평문으로 쓰던 문제**: 현재 폴더에 쓰는 Claude Code `.mcp.json` 에는
  `${LAW_OC}` 환경변수 참조를, VS Code `.vscode/mcp.json` 에는 `inputs` 비밀번호 입력(`${input:law-oc}`)을 넣는다.
  홈 디렉터리 설정(Claude Desktop·Cursor 등)은 커밋 대상이 아니라 종전대로 둔다

## [4.14.1] - 2026-09-23

성능·품질 기술부채 정리. 영역 4개(업스트림 I/O·체인·파싱·도메인 도구)를 병렬 리뷰해 찾은 결함을 한 번에 고쳤다.
실호출 A/B(같은 12개 시나리오, 구 4.14.0 대 신 빌드) 결과를 수치로 적는다. 테스트 806 → **981**, 테스트 시간 25.8초 → 약 10초.

### Performance

- **`get_law_text`(MST 단독)가 업스트림 5회에서 1회로**: 법제처가 eflaw 단건에 efYd 를 요구한 뒤(#153) MST 단독 eflaw 는 매번
  HTML 안내 페이지로 실패했고, 재시도 계층이 그 HTML 을 점검 페이지로 보고 4회(백오프 약 2.1초) 두드린 뒤에야 target=law 폴백에
  닿았다. 이제 MST 단독이면 target=law 로 바로 간다(결과는 폴백 본문과 같다). 실측: 조문 1개 3,065ms → **238ms**, 전문 4,007ms → **794ms**
- **`legal_analysis(verify_citations)` 가 다시 검증한다**: 위 낭비 때문에 인용 15건이면 요청 예산(업스트림 48회)이 바닥나 **✓0 / ⚠15**
  (전부 "예산 초과")였다. 법령명은 요청당 한 번만 풀고 동시성 8로 검증한다. 실측: 업스트림 48회 → **16회**, 결과 **✓15**
- `legal_analysis(impact_map)` 3,276ms → **590ms** (대상 조문 조회가 매번 4회 재시도 후 실패하던 경로 제거)
- `legal_research` 7종(`document_review` 제외)이 체인 데드라인을 쓰고, 기반 법령을 찾은 뒤의 독립 갈래는 동시에 출발한다
  (출력 순서는 그대로). 공용 `withChainDeadline` 헬퍼로 #131·#150 규칙을 한 곳에 모았다
- `search_law`: 시행예정 보조검색을 주 검색과 같이 출발시킨다. 0건 폴백은 그 약속을 재사용하고 폐지 이력 조회와 병렬로 기다린다
- 업스트림 시간 한도: 시도마다 30초를 새로 주던 것을 **전체 45초**로 묶었다(매달린 업스트림에 약 122초가 걸려 60초에 포기하는
  클라이언트가 오류 문구 없이 끊겼다). 헤더 뒤 본문이 멈추면 **청크 간 20초**에서 끊는다(종전 undici 기본 300초).
  law.go.kr 안티봇 프로브도 정지를 삼키지 않아, 정지 응답이 시도마다 두 번 기다리던 경로(재현 80초)가 45초 안에 끝난다
- 메모리·요청당 비용: kordoc 을 별표 파싱 때만 불러온다(유휴 RSS 약 114MB → 약 100MB). 요청마다 만들던 Ajv 검증기를 공유하고
  tools/list 스키마 변환을 한 번만 한다(tools/list 21회 뒤 RSS 126~130MB → 100~104MB). 파싱된 법령 전문 캐시를 전역 500건 캐시에서
  전용 12건 캐시로 뗐다. lawId 만 준 현행 본문 캐시는 1시간(개정 시행일을 넘겨 낡지 않게)
- `get_annexes`: 한 요청 안에서 같은 법령 전문(3.8MB)을 두 번 받던 것을 한 번으로

### Security

- **서버 API 키 노출 차단**: 법제처 검색 응답의 상세링크에는 그 요청에 쓴 OC 가 실려 오고, 여러 렌더러가 그 링크를 그대로 출력했다.
  자체 키 없는 사용자에게 서버 LAW_OC 가 보여 폴백 한도를 우회할 수 있었다. tool-registry 최종 게이트에서 모든 도구 출력의 키를
  가린다(`maskKeysInText`). 실측: 12개 시나리오 출력에서 원문 키 25회 → **0회**
- **ReDoS 3경로**: 인증 없는 요청 1건(100KB 이하)으로 이벤트 루프가 12~24초 멈췄다(`legal_research(document_review)` 의 금액·기간
  정규식, `get_annexes` 의 별표 표기 정규식, 검색어 정규화의 공백 백트래킹). 정규식을 선형으로 고치고(구·신 약 380만 건 퍼징에서 의도
  외 출력 차이 0) 모든 문자열 인자에 길이 상한을 건다(조문·별표 번호류 100자, 일반 2,000자, `text` 50,000자, 중첩 인자 포함).
  의도한 출력 차이: 계약서 분석은 라벨과 값 사이 500자를 넘는 간격을 더 짝짓지 않고, 쉼표만 든 쓰레기 금액(`", 원"`)을 만들지 않는다

### Fixed

- "실패를 0건으로" 계열(가장 위험한 유형: LLM 이 없는 결과를 근거로 답한다)
  - HTML 점검·안내 페이지 가드가 `searchLaw`·`getAnnexes` 에만 있어 행정규칙·자치법규 검색, 조문·법령 연혁, 신구대조, 3단비교가 점검
    페이지를 "검색 결과 없음"으로 냈다. 가드를 본문 읽기 단일 통로로 옮겼다. 따르면 더 나빠지던 `LAW_RESPONSE_TYPE=JSON` 우회 안내도 뺐다
  - 시행예정 보조검색 실패가 "시행예정 없음"으로 바뀌어 1시간 캐시됐고, `search_law_bulk` diff 는 그 법령을 "본문 동일, 생략"으로 넘겼다.
    이제 실패를 밝히고(결과는 1분만 캐시) diff 에선 "확인 실패, 재조회 필요"로 따로 보고한다. 확인된 법령이 0건이면 "변경 없음"이라 단정하지 않는다
  - impact_map 의 인용 검색 실패가 "0건"으로, cite_check 의 상세 조회 실패가 "✅ 계속 인용 추정"으로, full_research 의 기반 검색 장애가
    "법령 없음"으로 나가던 것 등 체인 곳곳의 무음 실패를 명시적 실패 표시로 바꿨다. 예산 소진·취소는 보조 조회가 삼키지 않는다(`rethrowIfFatal`)
  - `advanced_search` 가 5xx·타임아웃을 "0건"으로 냈고, 자치법규 항목 태그를 잘못 읽어 늘 0건이었다
- 잘못 읽던 응답 구조
  - `get_law_statistics`: 태그·필드명을 잘못 읽어 업스트림 31회 뒤 늘 "총 0건". 최신 날짜부터 돌고 MST 중복을 걷는다
  - 다자조약(`MultTrtyService`)을 못 읽었고 양자조약은 모든 필드가 N/A 였다. 검색 결과의 다음 단계 안내 파라미터도 틀렸다
  - 해석례 기간 검색이 한 페이지만 받아 로컬로 걸러 총건수가 틀렸다(2024년 "건축": 2건 → **55건**). 업스트림 `explYd` 로 넘긴다
  - `search_law` 의 시행예정 병기가 연혁 행에 창을 빼앗겨 시행령·시행규칙 개정을 놓쳤다(eflaw `nw=2`·100건). 도로교통법 시행규칙 2027-01-01 시행분이 이제 나온다
  - 생활용어 도구 2종이 존재하지 않는 조합의 target 을 불렀다(`dlytrm`·`dlytrmRlt` 로 교정, "월세" 0건 → 15건)
  - `get_article_history` 가 `제38조` 를 6자리 코드로 바꾸지 않아 늘 0건, 공정위 결정 사건번호(`</사건번호 >`) 유실, 영문 법령명 강조 태그 잔존,
    해석례·자치법규 라벨 오기, `search_historical_law` 의 폐지제정·날짜 표기 오류
  - 여러 래퍼가 업스트림 오류를 "판례를 찾을 수 없습니다" 같은 부존재 단정으로 덮어썼다
- 체인·시나리오
  - `full_research` 가 데드라인 때 이미 받은 해석례를 버리고 "위까지가 시간 안에 받은 전부"라고 했다
  - penalty 시나리오가 없는 파라미터로 전체 목차를 "벌칙 조항"으로 냈고 같은 전문을 두 번 받았다. 벌칙 장·과태료 조문만 골라 싣는다.
    delegation 시나리오의 죽은 갈래(질의를 무시하는 소스)는 빼고 "자동 대조 미제공" 안내로 바꿨다
- `get_article_detail`: 업스트림이 무시하는 항·호·목을 받은 조문에서 직접 거른다(종전엔 "제2항"이라 쓰고 조문 전체를 냈다)
- 판례 축약이 서로게이트 쌍을 자를 수 있었고 `[10]` 이상 참조 표시를 지웠다

- 독립 리뷰(컨텍스트 없는 리뷰어 3명) 반영: `advanced_search` 의 자치법규 ID 를 `get_ordinance` 가 받는 일련번호로, `search_decisions`
  해석례 기간에 숫자·구분자 날짜 허용(오류엔 필드명), 법령 통계가 전부 실패하면 첫 실패 원인 표기, 판례 부가 검색의 예산 소진이 이미 받은
  조문을 버리지 않게, 공백만 다른 같은 법령 검색을 두 번 치던 것, 키 마스킹이 뒤 문자열을 삼키던 것

### Changed

- SDK 가 요청을 거절한 사유(400 등)를 `[mcp] …` 로그로 남긴다. 봉투는 method 와 id·params 의 타입만 싣는다(값 미기록,
  method 의 제어문자는 치환). 2026-09-23 기준 claude.ai 커넥터 POST 의 약 15% 가 사유 미상의 400 이었다
- 해석례 본문(`get_decision_text` domain=interpretation)은 기본 축약에서 뺀다. 라벨을 "이유"로 바로잡자 축약기가 해석례의 핵심인
  이유를 절반 가까이 잘랐다(실측 4,608자 → 2,279자). 라벨 정정 전처럼 전문을 준다

## [4.14.0] - 2026-09-22

### Added

- **`get_admin_rule` 부분 조회 — `jo`·`chapter`·`keyword`·`page`** ([#162](https://github.com/chrisryugj/korean-law-mcp/pull/162)):
  행정규칙 중 전문이 긴 규칙은 뒷부분 장을 **영원히 읽을 수 없었다**. 법제처 admrul 상세는 법령(target=law)과 달리
  JO 파라미터가 없고 전문이 `<조문내용>` 통짜 1개 CDATA 로만 온다(외국환거래규정 기준 18.8만 자·조문 208개 — 실측).
  `get_admin_rule {id, jo:"제9-5조"}` 는 `jo` 를 무시하고 제1장부터 뱉다가 5만 자에서 잘렸다.
  - 서버가 전문을 조문 단위로 파싱해 4가지 뷰로 자른다 — 우선순위 `jo` > `chapter` > `keyword` > `page`,
    복수 지정 시 상위 하나만 적용하고 무시 목록을 응답에 명시
  - `jo` 는 `"제9-5조"`·`"9-5"`·`"제2-6조의2"`·`"9-5-2"` 를 모두 수용하고 `context` 로 전후 조문을 함께 준다.
    `keyword` 는 매칭 조문 **번호 전량**을 목록으로 주고 본문만 상위 N개로 줄인다(뒷장 조문이 목록에서 사라지면
    `jo` 로 이어 갈 단서가 없다). `page` 는 라인 경계 비중첩 청킹
  - 조문 헤더 판정은 `^` 앵커 + **조문번호 단조증가 검사** — 라인 시작 역행 참조(`제1-2조 제1항의 규정은…`)를
    새 조문으로 오분리하지 않는다. 조문 체계가 없는 항목식 훈령은 에러가 아니라 안내로 답한다
  - 전문 XML 을 `admrulxml:` 캐시(TTL 6h, LRU 20건)에 담아 `jo` → `keyword` → `page` 연속 조회의 재호출을 막는다
  - 파라미터 없는 전문 조회는 종전 동작 그대로. 다만 잘릴 때 부분 조회 힌트를 본문 **앞**에 넣는다(뒤에 붙이면 잘려 사라진다)
- **`compare_admin_rule_old_new` 제·개정이유 폴백**: 행정규칙 신구대조는 API 데이터가 없는 경우가 많아
  `[NOT_FOUND]` 다섯 글자만 나갔다. 대조 0건이면 같은 응답에 이미 들어 있는 `<제개정이유내용>` 을 발령번호·일자와
  함께 반환하고, 그것도 없으면 law.go.kr 안내로 끝낸다

### Fixed

- **부분 조회 파서의 조용한 라인 유실 2건** (#162 리뷰): 단위 테스트로는 안 잡히고 "성공했는데 내용이 없는" 유형이다.
  실데이터(외국환거래규정 564라인)로 커버리지를 재어 찾았다
  - 장 헤더 **뒤**에 오는 절 헤더(`제2장` 다음 줄의 `제1절 외국환은행`)가 `preamble` 로 빠졌는데, `preamble` 은
    `jo`·`chapter`·`keyword` 어느 뷰에서도 출력되지 않는다 → 첫 조문 이후의 떠도는 라인은 다음 조문 앞에 붙인다.
    수정 후 **564/564 라인 커버·preamble 0**
  - 장마다 조 번호가 1 로 리셋되는 체계(`제2장` / `제1조`)는 단조증가 검사에서 탈락한 뒤 `cur=null` 직후라
    본문에도 못 붙어 **조문이 통째로 사라졌다** → 장 헤더에서 단조증가 기준을 리셋한다
  - 빈 본문의 `페이지 1/0` 표기, 공백뿐인 `keyword` 가 `includes("")` 로 전 조문에 매칭되던 문제
  - 테스트 801 → **806**
...
근본 원인은 하나다 — **MST는 "버전"이 아니라 "공포본" 식별자**다. 조항별 시행일이 나뉜 공포본은 MST 하나에 시행 슬라이스가 여러 개 붙는데, 코드 세 곳이 MST를 버전 고유값으로 취급하고 있었다.

### Fixed

- **`applicable_law` 가 현행 버전을 오판정** (#152): `fetchHistoricalVersionsFull` 의 연혁 중복 제거 키가 MST 단독이라, lsHistory 가 같은 MST로 내보내는 시행일별 행 중 **내림차순 첫 행(미래 시행분)만 남고 이미 시행된 슬라이스가 사라졌다**. `efYd <= 기준일` 탐색이 그 MST 를 통째로 건너뛰어 직전 공포본을 "현행"으로 답했다 (실측: 형사소송법 @2026-08-19 → 시행 2026.6.24. 반환, 실제 현행은 시행 2026.7.1.). 키를 `MST:시행일` 쌍으로 교체
- **`get_law_text(mst=…)` 가 현행 아닌 버전에 `[NOT_FOUND]`** (#152): `lawService.do?target=eflaw` 는 MST 단독(시행일 미동반)으로 현행이 아닌 버전을 풀지 못하고 200 + `{}` 를 돌려준다 (실측: 시행 예정판 MST 288579·과거 연혁판 MST 280441 모두 빈 봉투). 법령 노드가 빈 응답이면 `target=law` 로 1회 폴백. 단, **시행일을 못박은 호출(`efYd` 동반)에는 폴백하지 않는다** — `target=law` 는 MST 만 받아 분리시행 공포본이면 마지막 슬라이스를 돌려주므로(실측: `target=law&MST=281865` → 시행 20271231, 시행 중인 20260701 이 아님), 폴백을 붙이면 `[NOT_FOUND]` 보다 나쁜 **조용한 다른-슬라이스 오답**이 되어 행위시법 조문 비교를 오염시킨다
- **`time_travel` 이 실재하는 개정을 "변경 없음"으로 단정** (연혁 dedup 수정의 파급): 두 시점이 한 분리시행 공포본 안에서 갈리면 `oldVer.mst === newVer.mst` 조기 종료가 걸렸다. 실제로는 내용이 다르다 (형사소송법 MST 281865 → 시행 20260701 판 조문 644개 / 20271231 판 653개). 시행일까지 같을 때만 "변경 없음"으로 판정
- **`time_travel` 의 시점별 본문이 뒤섞이던 경로**: 본문을 `target=law&MST` 로 집어, 분리시행 공포본이면 두 시점 모두 마지막 슬라이스를 받았다 — "시점 A: 2026.7.1. 시행" 머리말에 2027.12.31. 판 본문이 붙는다. 시행일을 동반한 `eflaw` 우선으로 교체하고 빈 봉투·시행일 부재 시에만 기존 경로로 물러선다

### Added

- **`applicable_law` 시행 예정 개정 경고** (#152): 이미 확보한 연혁의 미래 시행분에서 추출 — **추가 API 호출 0**. 현행 인용만 보고 답하면 곧 시행될 대개정을 통째로 놓친다 (실측: 형사소송법 2026.10.2. 시행 법률 제21857호 — 제215조의2 검사 직권 삭제, 제245조의7제3항 이의신청 3개월 기한 신설을 도구가 완전 침묵)

## [4.12.0] - 2026-08-17

법률 자문 3축(법적 정합성·토큰 효율·응답 성능) 통합 배치. [#150](https://github.com/chrisryugj/korean-law-mcp/pull/150) (@humdrum00001010) 을 머지해 이슈 **62건(#88~#149)** 을 닫고, 머지 전 도메인별 독립 리뷰에서 실측 재현된 **결함 31건을 후속 커밋으로 수리**했다. 테스트 196 → **701**.

### Changed (사용자 가시 출력)

- **날짜 표기 통일**: `2024.1.5.` → `2024.01.05`, 빈 시행일은 `""` → `"N/A"` (`formatDateDot` 단일 원본)
- **`discover_tools` 응답 형식**: 노출 도구는 이름+포인터만(설명 전문 재인쇄 제거), 티어 랭킹·섹션 상한 5 적용. 16개 intent 합계 **65% 감축**, 정답 잔존 10/10. 무매칭 응답도 전체 목록 대신 유사 카테고리로 축약

### Added

- **체인 데드라인** (`MCP_CHAIN_DEADLINE_MS`, 기본 45초, `src/tools/chain-deadline.ts`): 업스트림 꼬리에 이용자가 인질로 잡히지 않게 만료 시 **받은 갈래까지 조립해 부분 결과**를 돌려주고 못 받은 자리는 마커로 남긴다. 적용은 `legal_research` 8종 중 순차 사다리가 긴 `action_basis`·`full_research`·`dispute_prep` 3종. **기반 법령 탐색(프리픽스)부터 시계 안**이며, HTTP 모드는 부팅 시점 fail-fast 검증
- **별표 목록 페이지네이션** (`src/tools/annex-list.ts`): 업스트림 100건/페이지 창 밖의 별표에 도달(도로교통법 시행규칙 263건 중 별표28 본문 도달 실증). 페이지 상한·목표 도달·무진전 3중 중단
- **자치법규 조문 관련성 실측 대조** (`src/lib/ordinance-relevance.ts`): 상위 N건 본문을 실제로 열어 조문 인용을 확인 (옵트인 — 상위 10건·병렬 1.9초·예산 1/5 실측 근거로 기본 꺼짐)
- **판례 인용 검증 축** (`src/lib/case-citation.ts`): `verify_citations` 가 법령+판례 2축으로 확장. 실존불가/미확인을 구분하고, 실재 사건부호에 한해서만 단정

### Fixed

**부존재 오단정 — 실재 자료를 "없다"고 말하던 경로 (하드닝)**

- `fetch-with-retry`: 미스 확정 조건이 시도 순번(`attempt >= 1`)이라 **503·네트워크 오류 직후의 첫 빈 본문 1회 관측**만으로 확정됐다(에러 문안은 "200ms 재확인 후에도 동일"이라고 진술). 관측 이력 기준으로 교체
- `precedents`: HTML 폴백의 부존재 판정이 음성 신호(마커 부재)만으로 성립해, 점검·안티봇 변형 페이지에서 **실재 판례를 `[NOT_FOUND]` 로 단정**했다(종전 `EXTERNAL_API_ERROR` 대비 단정 강도 격상). 양성 신호를 요구하도록 수정
- `get_annexes`: HTML 응답이 무음으로 빈 목록이 되고 사다리 전멸 시 "법제처 DB에 없습니다"를 단정하던 경로 — `checkHtmlError` 적용 + 장애와 정상 0건 구분(`[UPSTREAM_NO_DATA]`). `ExecutionLimitError`·취소는 삼키지 않고 전파
- `api-client`: `singleRecordLookup` 을 endpoint 단위 → **target 화이트리스트**(`prec`/`thdCmp`/`lsStmd`)로. 빈 본문이 미스일 수 없는 target 까지 사다리가 잘리던 것 해소

**법적 정합성 (#90 #93 #95 #98 #116 #118)**

- `impact_map`: 조번호+법령명 2축 앵커로 제1032조 혼입·JO 코드 조용한 0건 반환 해소. 하드닝으로 **"구 OO법" 인용은 확정 배제가 아니라 보류**로 완화 — 위헌심판은 행위시법을 심사해 "구 매장및묘지등에관한법률 제5조 위헌소원" 같은 사건명이 흔한데 옛 공식 제명은 붙여쓰기라 약칭 판정에 걸리지 않는다(`군형법` vs `형법` 의 확정 제외는 유지). 제외 사유도 조문 축·법령 축으로 분리 표기
- `verify_citations`: 판례 인용 미추출로 검증이 법령에 한정되던 것 해소. 하드닝으로 수량 산문(`"2030도3000명 증가"`)이 사건번호로 추출돼 ✗ 단정되던 오탐 차단, 구 파산 부호 `파` 보강
- `cite_check`: 판시사항이 배열·`{#text}` 객체로 와도 문장으로 읽는다(종전 `"[object Object]"` 무음 실패). 입력과 다른 판례가 특정되면 경고 병기
- `time_travel`: diff 항목에 공포번호·개정일 병기, 변경 지점 중심 신구대조. 하드닝으로 목(目)·조문 직속 호를 스냅샷에 합산 — 목만 개정된 조문이 "본문 동일"로 무음 누락되던 것 해소
- `article-detail`: 의X 조문 위치 라벨의 조 중복(`제401조의2조`) 수정

**라우팅 (#99~#104 #119 #121~#123 #129 #130)**

- `specific_article` 선점으로 조문번호가 든 질의의 후행 의도가 폐기되던 것, 날짜 파싱이 시점 패턴보다 먼저 실행돼 트리거가 소실되던 것, `제` 생략 표기(`44조`)·다중 조문·별표번호·전문 요구가 추출에서 소실되던 것, `조례` 명시 없는 자치법규 질의가 폴백 체인에 못 들어가던 것
- **O(n²) 접두 패턴 제거 + query 길이 상한**(`.max(2000)`): 8.5k자 입력 476ms, 적대 입력에서 종전 최대 45.9초 → 1~8ms
- 하드닝: `statistics`(pri 9)가 `specific_article` 의 양보를 가로채 **"최근 개정된 근로기준법 제60조"가 법령·조문 없는 전역 통계로** 가던 결함, 접속어 제거가 경계 없이 적용돼 **"과학기술기본법" → "학기술기본법"** 으로 잘리던 결함, `dateRange` 주입이 `time_travel` 앵커를 덮어쓰던 결함, 지역 오인(`지역구`·`놀이기구`), 심판례 역선점, 고아 음절(`"헌재 결정문"` → `"문"`)

**성능·안정 (#88 #105 #112~#115 #128 #131)**

- 판례 미스 조회 12.5초·업스트림 5회 → **2.2초·2회**, 부존재를 단정하지 않는 `UPSTREAM_NO_DATA`
- `get_three_tier`/`get_law_system_tree` 미스 13.2초/11.7초 → ~2.1초/~2.4초, 체계도 히트 10.6~11.6초 → 6.3~8.3초(XML+JSON 병렬화 — XML 제거는 실측 대조로 불가 판명)
- **2MiB 초과 본문에서 300초 무한 정지 → 13ms 명시 에러**(v4.11.0 회귀), 예산 이중 청구 해소. 하드닝으로 안티봇 검사의 본문 이중 청구도 프리픽스 읽기로 전환
- 체인 기반 법령 탐색이 사용자 원문 그대로 AND 검색해 `NOT_FOUND` 로 중단되던 것 → 3단 해석 + 갈래 병렬(12~14초)

**응답·디스커버리 (#92 #102 #106~#111 #126 #145)**

- `truncateResponse` 한도가 50KB가 아니라 **5만 자(UTF-16 code unit)** 임을 명확히 하고 문장 중간 절단 해소. 하드닝으로 서로게이트 쌍 반토막 절단(옛 한자), 요약 모드 재절단 시 `📋 요약 모드` 꼬리가 잘려 무표기 절단이 되던 자기모순 해소
- `TOOL_ALIASES` 고아 키 4개로 한국어 별칭 10개가 증발하던 것, 별칭 양방향 부분일치로 `판례` 가 조세심판으로 해석되던 것, 카테고리 미등록 도구 13개, `chain_*` 2-hop 안내(동일 기능 1-hop 존재)

### 전제 정정·음성 결과

- 이슈 전제가 실측과 달라 고치는 대신 반증한 건: **#111**(한글 낱말 경계 통일은 유해 — `심판` 3섹션→0), **#117**(원인은 폴백이 아니라 업스트림의 조번호 무시), **#128**(lsStmd XML 호출 제거 불가 — 4개 법령 전수 대조), **#88**(HTML 오분류가 아니라 0바이트 empty 경로)
- **#147** 의 LexDiff 상류 불일치는 [chrisryugj/lexdiff#103](https://github.com/chrisryugj/lexdiff/issues/103) 으로 이관 후 상류를 ㊿ 범위로 수정 — 이식본 `citation-content-matcher.ts` 와 동작 재정합 완료

### 한계 고지

- 전 수치는 로컬 IP·소규모 n 기준이다. Fly 프로덕션 IP 에서는 안티봇 층이 개입하므로 절대값이 다를 수 있다(비율 논증은 유지)
- 체인 지연 꼬리 일부는 업스트림발이며(A/B로 코드 무죄 입증) 데드라인+부분 반환이 방어선이다
- 체인 갈래 병렬화로 법제처 방향 피크 동시성이 요청당 3 → ~9로 오른다. 로컬 실측에서 스로틀 징후는 없었으나 다중 사용자 조건은 미검증
- `route-patterns.ts`(규칙 테이블)는 200줄 규칙의 예외로 둔다

...
**법제처 42개 API를 10개 도구로.** 법령, 판례, 행정규칙, 자치법규, 조약, 해석례(국세청 포함) + **LLM 환각 방지 인용 검증(법령·판례, 실존+내용)** + **조문 영향 그래프** + **시점 비교 자동 diff** + **이럴 땐 이렇게 — 5단계 안내** + **판례 생사 확인(Citator)** + **행위시법 판단** + **조례 정비 레이더** + **폐지 법령 후속 규정 안내**를 AI 어시스턴트나 터미널에서 바로 사용.
...
## v4.14.1: 조문 조회 13배, 인용 검증 복구, 서버 키 노출 차단

성능·품질 기술부채를 한 번에 정리한 릴리스입니다. 같은 12개 요청을 구버전과 나란히 실행해 잰 값입니다. 테스트 806 → **981**.

- **조문 조회가 빨라졌습니다**: `get_law_text` 조문 1개 3.1초 → **0.24초**, 전문 4.0초 → **0.8초**. 법제처 쪽 변경(#153) 이후
  매 호출이 실패할 게 뻔한 요청을 4번 재시도한 뒤에야 본 요청으로 넘어가던 낭비를 없앴습니다
- **인용 검증이 다시 동작합니다**: 위 낭비 때문에 인용 15건을 검증하면 요청 예산이 바닥나 **15건 전부 "확인 불가"**로 끝났습니다.
  이제 업스트림 16회로 15건 모두 실존 확인(✓15). `impact_map` 은 대상 조문을 늘 못 찾던 문제가 풀려 3.3초 → 0.6초
- **서버 API 키가 응답에 노출되지 않습니다**: 판례·해석례 검색 결과의 상세링크에 서버 키가 그대로 실려 나가던 문제를 막았습니다
- **요청 하나로 서버를 멈추게 하던 입력 3종을 막았습니다**: 긴 공백·쉼표·숫자 반복으로 정규식이 12~24초 멈추던 경로를 선형으로 고치고,
  인자 길이 상한(일반 2,000자, 본문 50,000자)을 걸었습니다
- **실패를 "0건"으로 답하지 않습니다**: 점검 페이지·일시 장애·예산 초과가 "검색 결과 없음"이나 "변경 없음"으로 바뀌던 경로를 명시적 실패 표시로
  바꿨습니다. 다자조약·법령 개정 통계·해석례 기간 검색·생활용어·조문 연혁처럼 응답 구조를 잘못 읽어 늘 비거나 틀리던 도구도 고쳤습니다
- 체인 7종에 시간 한도와 병렬 갈래, 업스트림 전체 45초 한도, 유휴 메모리 약 14MB 절감. 자세한 내역은 [CHANGELOG](CHANGELOG.md)

---

## v4.14.0 — 긴 행정규칙의 뒷장을 드디어 읽는다

외국환거래규정처럼 전문이 긴 고시는 뒷부분 장을 **영원히 읽을 수 없었습니다**. 법제처 행정규칙 API 는 법령과 달리
조문 지정 파라미터가 없고 전문을 통짜 한 덩어리로만 주는데(외국환거래규정 18.8만 자·조문 208개), 응답 상한 5만 자에서
잘려 제2장 중반에서 끝났기 때문입니다. 이제 서버가 전문을 조문 단위로 파싱해 필요한 부분만 돌려줍니다. 테스트 763 → **806**.

- **`get_admin_rule` 에 부분 조회 4종**: `jo:"제9-5조"` (조문 한 개, `context` 로 전후 조문 동반) ·
  `chapter:"제9장"` (장 전체) · `keyword:"해외직접투자"` (매칭 조문 번호는 전량, 본문은 상위 N개) ·
  `page:2` (전문 페이징). `"9-5"`·`"제2-6조의2"`·`"9-5-2"` 같은 표기 흔들림을 모두 받습니다
- **추측할 자리를 남기지 않습니다**: 없는 조문은 수록 범위와 함께 `[NOT_FOUND]`, 조문 체계가 없는 항목식 훈령은
  에러 대신 "keyword 나 page 를 쓰라"는 안내. 파라미터 없이 부른 전문이 잘릴 때는 부분 조회 힌트를 잘려 나가지 않도록
  본문 **앞**에 붙입니다
- **신구대조가 없으면 제·개정이유로**: 행정규칙 신구대조는 API 에 데이터가 없는 경우가 많아 `[NOT_FOUND]` 다섯 글자만
  나갔습니다. 이제 같은 응답에 들어 있는 개정이유·주요내용을 발령번호와 함께 돌려줍니다

---

## v4.13.0 — 준법 등록부 감시: 98회 호출을 1~2회로

ISO 법규준수 등록부(품질·환경·안전보건·정보보안·BCM·부패방지·AI·ESG 8개 영역 108개 법령)를 이 서버로 주간 감시하던
실사용 보고 3건([#157](https://github.com/chrisryugj/korean-law-mcp/issues/157) · [#158](https://github.com/chrisryugj/korean-law-mcp/issues/158) · [#159](https://github.com/chrisryugj/korean-law-mcp/issues/159), @thkim660207-cmd)을 처리했습니다. 테스트 738 → **758**.

- **`search_law_bulk` — 법령명 배열을 한 번에**: 건당 `법령ID · MST · 시행일 · 시행예정` 한 줄만 돌려줍니다.
  `previous={법령ID: 직전 MST}` 를 주면 **MST가 달라진 법령만** 받는 diff 모드 — 등록부 전건 감시가 호출 1~2회로 끝납니다.
  응답 말미의 스냅샷 JSON을 다음 감시에 그대로 넣으면 됩니다. 공포됐으나 미시행인 개정은 MST가 아직 안 바뀌므로,
  "본문 동일 + 시행예정 있음"으로 따로 표시해 시행일을 놓치지 않게 합니다
- **개정 이력은 이제 opt-in**: `amendment_track` 이 제정 시점부터의 조문별 개정 전건을 항상 싣던 것을 껐습니다
  (`includeHistory=true` 로 복원). 산업안전보건법(1981년 제정) 실측에서 이 섹션 혼자 5만 자 응답 상한을 소진해,
  정작 필요한 **신구대조표가 잘려나가던** 문제입니다
- **이미지로만 온 별표는 그렇다고 말합니다**: 법제처가 배출허용기준 같은 수치 표를 `<img>` 태그로만 주는 고시가 있습니다.
  종전에는 "본문 있음"으로 그대로 나가 LLM이 수치를 추측하기 좋은 자리였습니다 — 이제 경고와 함께 원문 URL,
  그리고 **첨부파일 원문(hwpx/pdf) 링크**를 안내합니다

---

## v4.12.0 — 이슈 62건 통합 배치 + "없다고 잘못 말하던" 경로 차단

법률 자문 3축(법적 정합성·토큰 효율·응답 성능)을 실측해 등록 이슈 62건([#88~#149](https://github.com/chrisryugj/korean-law-mcp/issues))을 한 번에 해결한 배치([#150](https://github.com/chrisryugj/korean-law-mcp/pull/150), @humdrum00001010)와, 머지 전 도메인별 리뷰에서 찾은 결함 31건의 후속 수리. 테스트 196 → **701**.

**자료가 있는데 "없다"고 답하던 경로를 막았다.** 이 서버에서 가장 나쁜 실패는 느린 게 아니라 실재하는 법령·판례를 부존재로 단정하는 것이다.

- 판례 조회에서 **일시 장애(503·네트워크 오류) 직후의 빈 응답 한 번**을 부존재로 확정하던 판정을 관측 이력 기준으로 교체
- 법제처 점검·안티봇 페이지를 받았을 때 **실재 판례를 `[NOT_FOUND]`로 단정**하던 HTML 폴백 경로 차단 — 이제 기구 고장과 자료 부존재를 구분한다
- 별표 조회에서 HTML 응답이 조용히 빈 목록이 되고 "법제처 DB에 없습니다"로 굳던 경로 차단

**응답이 멈추거나 끊기지 않는다.**

- 체인 자문에 **45초 데드라인**(`MCP_CHAIN_DEADLINE_MS`) — 만료되면 받은 갈래까지 조립해 **부분 결과**를 돌려주고 못 받은 자리는 마커로 남긴다. 업스트림이 느릴 때 MCP 클라이언트 타임아웃(60초)에 걸려 통째로 날리던 것이 사라졌다
- 2MiB 초과 본문에서 **300초 무한 정지 → 13ms 명시 에러**(v4.11.0 회귀 수정)
- 판례 미스 조회 12.5초 → **2.2초**, 체계도 히트 10.6~11.6초 → 6.3~8.3초
- 8.5k자 질의의 라우팅 476ms, 적대적 입력에서 최대 45.9초 걸리던 정규식 제거

**인용 검증이 더 정확해졌다.**

- `verify_citations`가 법령+**판례** 2축으로 확장 — 실존 불가와 미확인을 구분해 표기한다
- `impact_map`이 조번호에 더해 **법령명까지** 대조 — 형법 제1조 질의에 군형법 제1조가 섞이던 것 차단. 판정이 애매하면 버리지 않고 보류한다(위헌심판의 "구 OO법" 인용 포함)
- `cite_check`가 판시사항을 배열·객체 형태로 받아도 읽는다(종전에는 조용히 실패)

**별표·검색.** 100건 창 밖의 별표에 도달(도로교통법 시행규칙 263건 중 별표28 본문 확인), 별표 `1의2`를 별표 1로 조용히 바꿔 주던 오선택 차단, `discover_tools` 응답 65% 감축(정답 잔존 10/10).

> 사용자 가시 변경 2건: 날짜 표기가 `2024.1.5.` → `2024.01.05`로 통일(빈 시행일은 `N/A`), `discover_tools` 응답이 포인터·랭킹 형식으로 바뀌었다.
...
> ⚠️ Breaking: HTTP 바인드 기본값이 `0.0.0.0` → `127.0.0.1`, `TRUST_PROXY` 기본값이 `1` → `false`(허용값도 1~10 정수만), `get_batch_articles` 입력 상한(법령 20개·법령당 조문 50개·요청당 100개). 자세한 마이그레이션은 [CHANGELOG](./CHANGELOG.md#4110---2026-08-16) 참조.

## v4.10.0 — 폐지된 법령을 찾을 때 후속 규정을 알려준다

폐지된 법령명으로 검색하면 0건만 돌아오던 것을, **연혁을 추적해 폐지 사유와 후속 통합 규정을 안내**하도록 바꿨다. 법령·행정규칙 양쪽 모두 지원한다. "지금은 없는 법"을 묻는 질문이 막다른 길로 끝나지 않는다.
...
v3는 같은 41개 API를 **14개 도구**로 재압축했습니다 (v3.2.2 이후 15개, v4.3에서 19개, v4.4.0에서 통폐합으로 9개, 현재 10개).
